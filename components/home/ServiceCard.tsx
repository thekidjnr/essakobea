...
export default function ServiceCard({
  ...
  const onPointerUp = (e: React.PointerEvent) => {
    ...
    if (!endDrag()) router.push(`/book?service=${slug}`);
  };
  ...
  return (
    <div className="group cursor-pointer">
      <div
        ...
        onKeyDown={(e) => {
          if (e.key === "Enter" || e.key === " ") router.push(`/book?service=${slug}`);
        }}
          ...
          {images.map((src, i) => (
            <div key={src + i} className="relative h-full" style={{ width: `${100 / images.length}%` }}>
              <Image
                ...
                fill
                draggable={false}
                className={`object-cover ${position} transition-transform duration-700 ${dragging ? "" : "group-hover:scale-105"}`}
